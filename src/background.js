...
var cm_clickHandler = function(clickData, tab) {
    chrome.tabs.create({ url: "http://pan.baidu.com/s/" + clickData.selectionText.trim()
        + (pass ? ('?pass=' + pass) : '') });
};

var cm_searchClickHandler = function(clickData, tab) {
    var se = clickData.selectionText.trim();
    chrome.tabs.create({ url: 'http://www.sobaidupan.com/search.asp?r=0&wd=' + encodeURIComponent(se) });
};
    ...
    if (msg.request === 'updateContextMenu') {
        ...
        if( !se.match(/^[RBV]J\d+$/i)) {
            ...
            if( match && !match[2].match(/^[0-9]+$/)) {
                ...
                if( msg.context ) {
                    ...
                    if( index != -1 ) {
                        var afterSelectionText = msg.context.substr(index + match[2].length);
        ...
        if( isBaiduPanCode ) {
            ...
            if( pass )
                o.title += ' (密码: ' + pass + ')';
            if( cmid == null )
                cmid = chrome.contextMenus.create(o);
            else
                chrome.contextMenus.update(cmid, o);
        } else {
            if( cmid == null )
                cmid = chrome.contextMenus.create(clone(optionsSearch));
            else
                chrome.contextMenus.update(cmid, clone(optionsSearch));
        }
    }
});
        ...
        if( pass ) {
            for(var i =0; i < details.responseHeaders.length; i++) {
                if( details.responseHeaders[i].name.toLowerCase() == 'location' ) {
                    if(details.responseHeaders[i].value.indexOf('?shareid=') != -1
...
}, {urls: ["*://pan.baidu.com/s/*"], types: ["main_frame"]}, ["blocking", "responseHeaders"]);
